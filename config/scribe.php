...
return [
    ...
    'description' => 'REST API v1. Every response uses the envelope { status, data, errors, hasError, message }.',
    ...
    'intro_text' => <<<'INTRO'
            ...
            Errors use `"status": "fail"`, `"hasError": true` and put the messages in `errors`
            (a list of strings, or `{ "field": ["message"] }` for validation errors, HTTP 422).

            List endpoints accept `page`, `per_page`, `order_by`, `order_type`, `keyword`, `select[]`,
            `with`, `group_by`, `return_type=count` and `operations[]` (see the README).
        INTRO,
    ...
    'auth' => [
        ...
        'extra_info' => 'Get a token from <code>POST /api/v1/auth/login</code> (or register) and send it as <code>Authorization: Bearer {token}</code>.',
    ],
